...
public class EmployeeDetailRouteController extends BaseRouteController {
    ...
    public ModelAndView start(
        ...
        if (Employee = null || this.isElevatedUser(activeUserEntity.get())) // no employees exist or current user is elevated
        {
            //Employee Detail view/document
        }
        else if (!activeUserEntinty.isPresent()) // no active user for current session
        {
            // redirect to Sign In view/document with error message
        }
        else
            // redirect to Main Menu view/document with error message
